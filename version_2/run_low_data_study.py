...
def evaluate_plain_deep_models(
    ...
    for spec in model_specs:
        ...
        pred_scaled = model.predict(spec["test_inputs"], verbose=0)
        predictions = scaled["scaler_y"].inverse_transform(pred_scaled)
        hot_pred = predictions[:, 0]
        cold_pred = predictions[:, 1]

        hot_metrics = compute_metrics(y_test[:, 0], hot_pred)
        cold_metrics = compute_metrics(y_test[:, 1], cold_pred)

        hot_capacity_rate = float(DEFAULT_CONFIG["assumed_hot_mass_flow_kg_s"] * DEFAULT_CONFIG["cp_hot_kj_kgk"])
        derived_heat_load_pred = hot_capacity_rate * (
            test_frame["hot_inlet_temperature_k"].to_numpy(dtype=float) - hot_pred
        )
        actual_heat_load = test_frame["hx_1_heat_load_kw"].to_numpy(dtype=float)
        heat_load_metrics = compute_metrics(actual_heat_load, derived_heat_load_pred)
        ...
        save_deep_artifact(
            ...
            sequence_length=PILSTM_SEQUENCE_LENGTH,
            metrics={
                "hot_outlet_temperature_k": hot_metrics,
                "cold_outlet_temperature_k": cold_metrics,
                "hx_1_heat_load_kw": heat_load_metrics,
            },
            notes=spec["notes"],
        ...
        metric_rows.append(
            metric_row(
                ...
                split_strategy="Shared low-data sequential split for deep comparison",
                n_train_rows=len(X_train),
                n_val_rows=len(X_val),
                n_test_rows=len(X_test),
                metrics=hot_metrics,
        ...
        prediction_output_rows.extend(
            prediction_rows(
                ...
                model_name=spec["model_name"],
                frame=test_frame,
                actual_values=actual_heat_load,
                predictions=derived_heat_load_pred,
            )
        )
