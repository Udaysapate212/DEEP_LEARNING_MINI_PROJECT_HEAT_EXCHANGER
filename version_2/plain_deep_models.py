...
def build_mlp_sequence_model(sequence_length: int, n_features: int) -> "keras.Model":
    model = keras.Sequential(
        [
            ...
            layers.Dropout(0.15),
            layers.Dense(len(OUTPUT_FEATURES)),
        ]
    )
...
def build_vanilla_lstm_model(sequence_length: int, n_features: int) -> "keras.Model":
    ...
    outputs = layers.Dense(len(OUTPUT_FEATURES))(x)
...
def restore_deep_model(artifact_path: Path, weights_path: Path) -> dict | None:
    ...
    artifact.setdefault("output_features", list(OUTPUT_FEATURES))
    ...
    try:
        ...
        print(
            f"Warning: failed to load weights from {weights_path}. "
            "Attempting partial load with skip_mismatch=True."
        )
        try:
            model.load_weights(str(weights_path), by_name=True, skip_mismatch=True)
        except Exception:
            print(f"Error: unable to restore deep model weights from {weights_path}: {exc}")
            return None
